...
export async function GET(request: Request) {
  ...
  if (!id) {
    return NextResponse.json(
      { message: "Не указан идентификатор приемки" },
      { status: 400 }
    );
  }

  try {
    const url = new URL(`${ONE_C_API_URL.replace(/\/+$/, "")}/PrintInvoice`);
    ...
    console.error("Failed to load printable invoice PDF from 1C", error);
    return NextResponse.json(
      { message: "Не удалось получить печатную форму приемки из 1С" },
      { status: 502 }
    );
  }
}
