...
# заданного в минутах. Используется из Makefile (`autodeploy`,
# `_checkout-branch`, `_cron-sync-comment`) — все они пересобирают
# cron-строку через этот скрипт, чтобы выбранный пользователем интервал
# не сбрасывался обратно на дефолтный час.
#
# < 60          -> "*/N *"      (каждые N минут)
